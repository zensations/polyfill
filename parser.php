...
class CssParser {
			...
			switch($this->state){
				...
				case 'at';
					...
					if($this->css{$i} == '{'){
						...
						$this->current['se'] = null;
						$this->current['pr'] = null;
						$this->current['va'] = null;
						$this->token = '';
					}
					else {
						$this->token .= $this->css{$i};
	...
	protected function merge(){
		...
		if($se == '@font-face'){
			...
			if(empty($se) || empty($va) && empty($pr) ) {
				if(!array_key_exists($at, $this->parsed)){
					$this->parsed[$at] = array();
				}
				if(empty($va) && empty($pr)){
					if(!empty($se) && !array_key_exists($se, $this->parsed[$at])){
						$this->parsed[$at][$se] = array();
					}
					return;
				}
				return;
